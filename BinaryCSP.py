...
def neighbors(assignment, csp, var):
	bConstraints = csp.binaryConstraints
	neighbor = []
	
	for i in bConstraints:
		if i.affects(var):
			oV = i.otherVariable(var)
			if assignment.isAssigned(oV):
				continue
			else: 
				neighbor.append(i.otherVariable(var))
	return neighbor

def num_constraints(binaryConstraints, var):
    count = 0
...
def consistent(assignment, csp, var, value):
	"""Question 1"""
	"""YOUR CODE HERE"""
	assignedVal = assignment.assignedValues
	binConstraints = csp.binaryConstraints

	for i in binConstraints:
		if i.affects(var):
			oV = i.otherVariable(var)
			if assignment.isAssigned(oV):
				oVal = assignedVal[oV]
				temp = i.isSatisfied(oVal, value)
				if temp:
					continue
				else:
					return False
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	...
	if assignment.isComplete():
		return assignment
 		...
 		orderVal = orderValuesMethod(assignment, csp, var)
 		for val in orderVal:
 			inferences = None
 			temp = consistent(assignment, csp, var, val)
 			if temp:
 				assignedVal[var] = val
 				...
 				if inferences == None:
 					...
 					toReturn = recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod)
 					if toReturn != None:
 						return toReturn
 			...
 			if var in assignedVal:
 				assignedVal[var] = None
	...
	for d in domains:
		if assignedVal[d] == None:
			...
			if nextVar == None:
				...
				smallConstraints = num_constraints(binConstraints, nextVar)
				dConstraints =  num_constraints(binConstraints, d)
				if dConstraints > smallConstraints:
					nextVar = d
					varLen = dLen
...
def forwardChecking(assignment, csp, var, value):
	...
	for k in binConstraints:
		if k.affects(var) == None:
			if assignedVal[k.otherVariable(var)] == None:
				oV = k.otherVariable(var)
				pruned.append(oV)
...
def revise(assignment, csp, var1, var2, constraint):
	...
	domains = assignment.varDomains
	flag = 0
	newDom = [] #since var1 is consistent a new domain is not created!
	
	for i in domains[var2]:
		newDom.append(i)

	for j in newDom:
		flag = 0
		for k in domains[var1]:
			iSa = constraint.isSatisfied(j,k)
			if iSa:
				flag = 1	
				
		if flag == 0:
			inferences.add((var2, j))
			domains[var2].remove(j)
			domLen = len(domains[var2])
			if (domLen == 0):
				for p, pval in inferences:
					domains[p].add(pval)
				return None
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	while queue.qsize() > 0:
		x1, x2, constraint = queue.get()
		
		prevLen = len(domains[x2])
		toReturn = revise(assignment, csp, x1, x2, constraint)
		currLen = len(domains[x2])
		
		if toReturn == None:
			for p, pval in inferences:
				if pval in domains[p]: 
					continue
				else:
					domains[p].add(pval)
			return None
			...
			inferences = inferences.union(toReturn)
				...
				if currLen == 0:
					for p, pval in inferences:
						if pval in domains[p]: 
							...
							domains[p].add(pval)
					return None 
			# using a temp function to get neighbors of x2	
			neighbor = neighbors(assignment, csp, x2)
			...
			if len(toReturn) != 0:
				for k in neighbor:
					for i in binConstraints:
						oV = i.otherVariable(var)
						oV2 = i.otherVariable(k)
						
						if ((i.affects(k)) and (oV2 == x2)):
							if not assignment.isAssigned(oV):
								queue.put((x2, k, i))

	for p, pval in inferences:
		if pval in domains[p]:
			domains[p].remove(pval)
...
def AC3(assignment, csp):
	...
	while not queue.empty():
		currVar = queue.get()
		for c in binConstraints:
			if c.affects(currVar):
				oV = c.otherVariable(currVar)
				newI = revise(assignment, csp, currVar, oV, c)

				if newI == None:
					return None
				else:
					for i in newI:
						if not i in inferences:
							inferences.add(i)
							queue.put(i[0])
